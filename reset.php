...
<style>
 ...
 <div class='col-md-4 bg-light rounded' style='margin-top:10em;'>
         ...
         <input type="password" required name="pwd" class="form-control" placeholder="********">
            ...
            <div class="form-group input-group">
                    <span class='input-group-text bg-white'style='border:none;border-bottom:1px solid lightgray;border-radius:0px;'><i class='icofont icofont-lock'></i></span>
         <input type="password" name="cpwd" class="form-control" placeholder="********">
            </div>
            <div class="form-group input-group">
             <input type="submit" required name="save" class="btn btn-block btn-outline-light" value="Reset Password" style='  background-image:linear-gradient(to left,rgba(200,0,200,0.6),rgba(0,0,0,0.9));border-radius:100px;margin-top:2em;'>   <div class='text-center pt-3'>
                
                <span style='font-family:century gothic;'><a href='login.php'>click here to login</a></span>
                </div>
                </div></form>
         <?php
    if(isset($_POST['save']))
    {
        ...
        if($pwd==$_POST['otp'])
        {
            ...
            {
                $sql="update users set password='$_POST[pwd]'where email='$_GET[id]'";
